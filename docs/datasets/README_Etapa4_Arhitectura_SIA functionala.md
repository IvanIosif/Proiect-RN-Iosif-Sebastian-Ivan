...
|:---------------------------------------------------------------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------|:--- |
| Triajul clinic rapid în zone rurale izolate, fără acces la radiologie, pentru decizia de transfer. | Clasificarea automată a 20 de indicatori clinici cu un timp de răspuns de **< 1 secundă** pentru recomandarea de transfer.    | **Modul 2 (RN)** + **Modul 3 (UI Streamlit)** |
| Identificarea precoce a cazurilor de Tuberculoză (T) pentru a preveni contagiunea în comunitate.   | Analiza corelațiilor între simptomele "ancoră" (pierdere kg, tuse cu sânge) pentru o **acuratețe de detecție > 92%**.         | **Modul 2 (Neural Network)** |
| Optimizarea resurselor de transport medical (ambulanțe) la nivel județean.                         | Reducerea cu **25% a transferurilor inutile** prin confirmarea cazurilor de Pneumonie ce pot fi tratate la dispensarul local. | **Modul 1 (Data Logging)** + **Modul 3 (UI)** |
...
**Total observații finale:** 30,000 (după Etapa 3 + Etapa 4)
**Observații originale:** 30,000 (100%)

**Tipul contribuției:**
[X] Date generate prin simulare fizică (Simulare clinică probabilistică)
[ ] Date achiziționate cu senzori proprii  
[ ] Etichetare/adnotare manuală  
[ ] Date sintetice prin metode avansate

**Descriere detaliată:**
Întreg setul de date de 30,000 de observații este rezultatul unei simulări clinice originale, concepute pentru a modela diagnosticul diferențial între Pneumonie și Tuberculoză în contextul triajului rural. Generarea a fost realizată programatic, utilizând distribuții de probabilitate multinominale pentru fiecare dintre cele 20 de simptome. Am definit două profiluri clinice distincte: unul pentru Pneumonie (evoluție acută, febră ridicată, tuse productivă) și unul pentru Tuberculoză (evoluție cronică, pierdere în greutate, transpirații nocturne, tuse cu sânge).

Fiecare eșantion a fost generat prin aplicarea unui "zgomot" probabilistic de ~15%, simulând cazuri medicale atipice unde simptomele se suprapun (feature overlap). Această metodă este relevantă pentru problemă deoarece permite antrenarea unei rețele neuronale pe un volum masiv de date (30k), imposibil de obținut prin colectare manuală în timp scurt, asigurând totodată diversitatea statistică necesară pentru o generalizare robustă în condiții reale de dispensar rural.
...
Diagrama vizuală a fost salvată în locația: D:\Facultate\RN\docs\state_machine.png

IDLE → ACQUIRE_SYMPTOMS → VALIDATE_INPUT →
PREPROCESS (Scaling 1-5 to 0-1) → RN_INFERENCE (Analysis) →
TRIAGE_DECISION →
├─ [Probabilitate T > 0.5] → TRIGGER_TRANSFER_ALERT → LOG_DATA → IDLE
└─ [Probabilitate P <= 0.5] → DISPLAY_LOCAL_TREATMENT → LOG_DATA → IDLE
↓ [Eroare date/Sistem]
ERROR_STATE → NOTIFY_USER → IDLE

Am ales arhitectura de tip Clasificare la punct de îngrijire (Point-of-Care) cu logică de triaj în timp real pentru că proiectul răspunde nevoii critice de a separa pacienții cu Pneumonie de cei cu Tuberculoză în zone rurale, unde resursele de transport medical sunt limitate.

Stările principale sunt:

    IDLE: Sistemul se află în așteptare, pregătit pentru un nou pacient.
    ACQUIRE_SYMPTOMS: Utilizatorul introduce scorurile (1-5) pentru cele 20 de simptome clinice prin interfața Streamlit.
    PREPROCESS: Modulul preia datele brute și aplică normalizarea Min-Max pentru a le aduce în intervalul [0, 1], necesar funcției de activare Sigmoid a rețelei neuronale.
    RN_INFERENCE: Rețeaua neuronală procesează vectorul de caracteristici și returnează o probabilitate de diagnostic.
    TRIAGE_DECISION: O stare logică de decizie care interpretează rezultatul RN și oferă o recomandare medicală clară (Transfer vs. Tratament local).
    Tranzițiile critice sunt:

    ACQUIRE_SYMPTOMS → VALIDATE_INPUT: Verifică dacă toate cele 20 de câmpuri au fost completate. Dacă nu, trece în ERROR_STATE.

    RN_INFERENCE → TRIAGE_DECISION: Tranziția se face imediat ce modelul a generat valoarea de ieșire (latență < 1s).

    TRIAGE_DECISION → LOG_DATA: Salvează automat cazul în format CSV (Modul 1) pentru raportări epidemiologice ulterioare, înainte de a reveni în starea IDLE.

    Starea ERROR_STATE este esențială deoarece, în context medical, datele incomplete sau incoerente pot duce la diagnostice greșite. Această stare previne afișarea unui rezultat dacă valorile introduse nu respectă domeniul definit [1, 5], asigurând siguranța actului medical în triajul rural.
...
Acest script generează dataset-ul de 30.000 de eșantioane cu cele 20 de simptome, respectând profilurile clinice de Pneumonie și Tuberculoză.
